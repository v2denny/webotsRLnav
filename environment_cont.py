...
class WebotsEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # CHANGED: Using continuous action space instead of discrete
        # First value: forward only (no backward), Second value: rotation
        self.action_space = spaces.Box(
            low=np.array([0, -1]),  # Lower bound: 0 for forward (no backward), -1 for rotation
            high=np.array([1, 1]),  # Upper bound: 1 for forward, 1 for rotation
            dtype=np.float32
        )
        ...
        # CHANGED: We track previous control values instead of discrete actions
        self.prev_controls = []
    ...
    def step(self, action):
        ...
        # CHANGED: Check if moving forward would lead to collision for continuous actions
        # Only check when forward motion is significant
        if action[0] > 0.3:  # If trying to move forward significantly
        ...
        # CHANGED: Scale continuous actions to actual robot control values
        # Only positive values for forward movement (no backward)
        forward_speed = action[0] * 0.12  # Scale to match the original discrete values (0.12)
        rotation_speed = action[1] * 1.3  # Scale to match the original discrete values (1.3)

        # Execute the action
        cmd_vel(self.supervisor, forward_speed, rotation_speed)
        self.supervisor.step(200)

        # Store current control values for reward calculations
        self.current_forward = forward_speed
        self.current_rotation = rotation_speed
        self.prev_controls.append((forward_speed, rotation_speed))
        if len(self.prev_controls) > self.action_history_size:
    ...
    def calculate_reward(self):
        ...
        # CHANGED: Consistent turning incentive for continuous actions
        consistent_turning_reward = 0
        if self.same_spot_steps > 4:
            # Check if robot has been consistently turning in the same direction
            if abs(self.current_rotation) > 0.5:  # If currently turning significantly
                rotation_sign = np.sign(self.current_rotation)
                same_direction_turns = sum(1 for control in self.prev_controls
                                           if np.sign(control[1]) == rotation_sign and abs(control[1]) > 0.5)
...
class MetricsTracker:
    def __init__(self):
        ...
        # CHANGED: No need to track discrete action frequencies for continuous actions
        # self.actions = []
    ...
    # CHANGED: Removed log_action method as we're not tracking action frequencies
    ...
    # CHANGED: Removed calculate_percentages method as we're not tracking action frequencies
    ...
    def get_summary(self):
        ...
        return {
            ...
            # CHANGED: Removed actions percentage from summary
